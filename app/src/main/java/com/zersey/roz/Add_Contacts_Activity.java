...
public class Add_Contacts_Activity extends AppCompatActivity {
	...
	private EditText ContactNameEdit, ContactNumberEdit, ContactAddressEdit, ContactEmailEdit,
		ContactCompanyEdit;
	private ImageButton Contact_Delete_Button;
	private String Updated_Name, Updated_Number, CardClicked;
	private int Updated_Id;
	private MaterialTextField Material_Name, Material_Number, Material_Address, Material_Email,
		Material_Company;
			...
			if (TextUtils.isEmpty(CardClicked)) {
				if (Check_Contacts(Number)) {
					...
					if (newRowId == -1) {
						...
						Toast.makeText(Add_Contacts_Activity.this, "Error with saving Contact",
							Toast.LENGTH_SHORT).show();
					} else {
						// Otherwise, the insertion was successful and we can display a toast with the row ID.
						Toast.makeText(Add_Contacts_Activity.this,
							"Contact saved with row id: " + newRowId, Toast.LENGTH_SHORT).show();
					}
					Intent intent =
						new Intent(Add_Contacts_Activity.this, Contact_List_Activity.class);
				...
				if (Check_Contacts(Number)) {
					...
					db.update(Contacts_contract.Contacts_Entry.Table_name, values,
						Contacts_contract.Contacts_Entry._id + "=" + Updated_Id, null);
	...
	public void Delete_Button(View view) {
		...
		SQLiteDatabase db = mdbhelper.getWritableDatabase();
		// ContentValues values = new ContentValues();
		db.delete(Contacts_contract.Contacts_Entry.Table_name,
			Contacts_contract.Contacts_Entry._id + " = ?", new String[] { "" + Updated_Id });
